...
router.get("/estatura", async (req, res) => {
    try {
        const resultado = await promedioEstatura();
        res.json(resultado);
    } catch (error) {
        console.error(error);
        res.status(500).json({
...
router.get("/peso", async (req, res) => {
    try {
        const resultado = await promedioPeso();
        res.json(resultado);
    } catch (error) {
        console.error(error);
        res.status(500).json({
...
router.get("/hipertension", async (req, res) => {
    try {
        const resultado = await hipertension();
        res.json(resultado);
    } catch (error) {
        console.error(error);
        res.status(500).json({
...
router.get("/hipertension-peso", async (req, res) => {
    try {
        const resultado = await relacionHipertensionPeso();
        res.json(resultado);
    } catch (error) {
        console.error(error);
        res.status(500).json({
...
router.get("/sobrepeso-trabajo", async (req, res) => {
    try {
        const resultado = await relacionSobrepesoTrabajo();
        res.json(resultado);
    } catch (error) {
        console.error(error);
        res.status(500).json({
...
router.get("/intervalo-estatura", async (req, res) => {
    try {
        const nivelConfianza = Number(req.query.confianza || 0.95);

        if (nivelConfianza <= 0 || nivelConfianza >= 1) {
            return res.status(400).json({
                error: "El nivel de confianza debe estar entre 0 y 1."
            });
        }

        const resultado = await intervaloConfianzaEstatura(nivelConfianza);
        ...
        res.status(500).json({
            error: "Error calculando el intervalo de confianza."
        });
    }
});
...
router.get("/prueba-imc", async (req, res) => {
    try {
        const imcPoblacional = Number(req.query.imc || 25);
        const nivelSignificancia = Number(req.query.alpha || 0.05);

        if (imcPoblacional <= 0) {
            return res.status(400).json({
                error: "El IMC poblacional debe ser mayor que 0."
            });
        }

        if (nivelSignificancia <= 0 || nivelSignificancia >= 1) {
            return res.status(400).json({
                error: "El nivel de significancia debe estar entre 0 y 1."
            });
        }

        const resultado = await pruebaHipotesisIMC(
            imcPoblacional,
            nivelSignificancia
        );
        ...
        res.status(500).json({
            error: "Error realizando la prueba de hipótesis."
        });
    }
});
